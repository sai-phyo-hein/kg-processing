    ...
    def _get_by_canonical_ids(
        ...
        for point in points:
            ...
            results.append({
                ...
                # name_en: English translation of the canonical name, if
                # the registry payload actually has one — read
                # defensively (falls back to "" rather than duplicating
                # "name") since whether this field exists depends on the
                # live Qdrant schema. Threaded through to preprocessor.py's
                # entity_details/predicate_details and from there into
                # aggregator.py's canonical link lines (see
                # _canonical_link_lines), which render
                # "raw -> (linked) -> canonical [canonical_en]" when
